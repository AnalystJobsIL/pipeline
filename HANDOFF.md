...
- **2026-09-02 `roles`** - a retraction bound to a LIVE role through another posting's stray `seen_id` (`545`); a fold the ledger had recorded re-collided in `merged` every morning. Own-url binding, read from BOTH stores (a wave proved a ledger-only set fails OPEN); a settled twin group drops instead of skipping. Reds closed: `558@registry`, `564@jd-text`. **NOT finished:** `555`-`557`, `565`. Record: `docs/sessions/2026-09-02-roles.md`.
