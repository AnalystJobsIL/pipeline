...
| 2026-09-19 | registry | **the push is green on a runner.** `gh run view 35359064839 --json jobs` (headSha `267e6ac`, the last commit of this session) reads **16 of 16 success** at job level — in particular `guard-kill` and all eight `mutation-gate` shards, which carry the seven records this branch re-anchored or filed. Any red is NAMED with its owner; a `mutation-gate` red naming one of `ledger-refuses-*`, `hunt-*-ledger-*`, `queue-lapse-suppression-*`, `search-backoff-uncapped` or the three re-anchored `hunt-keep-address-*` is **registry's**, not inherited | | |
...
| 2026-09-19 | roles | first `event: schedule` digest on a headSha containing the reject sweep: `cloud_state/roles.csv` holds **0** rows with `class_decision=reject` (was 11), `Stages:` names >=7 rows whose reason starts `classifier ` , `dataset 173 roles`, meta `excluded.withdrawn` **74** (simulated over `37f308a`: 7 hand lines + 7 machine) | | |
| 2026-09-19 | roles | the same digest's `Roles:` line carries `closure text on 2 board-listed row(s) (ignored)`, and `closed by page` does NOT name HiBob or Meta — 0 closures is the correct output | | |
| 2026-09-19 | roles | `Roles:` carries `twin folds 1`; `roles.csv` holds exactly one BioCatch BI-developer row, the one keyed on the plain title (not the `senior` one, which supersedes); `ledger N = store N` | | |
| 2026-09-19 | roles | **no** `alias fold left` line and **no** `alias folds: N renamed` line in the digest — the no-twin rename is measured empty and must stay so until a pair carries both declarations. Registry's `de531b1` parked BOTH Harel rows, so no Harel fold is possible; a rename line here is a fold nobody declared | | |
...
- **2026-09-18 `registry`** — a NOT-THEIRS ledger read bound nothing, so the hunt re-activated Kima and PayPlus. `board_verify.refuses` now vetoes both hunt arms and `--verify-existing`; `ledger_contradicted` **3→0**. Lapse alarm fixed; back-off 14→28→56/90 (09-29 **225→123**). Harel board found, gate refuses (`621`). BD **2**, seam **2**. Local **2060 passed**; CI `35359064839` **1/16, queued** (09-19 row). **NOT finished:** `621`, `622`, `459`. Record: `docs/sessions/2026-09-18-registry.md`.
- **2026-09-18 `classifier`** — the 2026-09-14 data-platform ruling sat in no contract and no record, and 11 published rows carried a `reject` cell nothing could flip back. Record written; contract bumped on a measured 2-of-20 flip set; `reject_owed` pool + `ADJUDICATED`; 7 withdrawn, 6 kept. 74 calls, 0 BD. CI PENDING. **NOT finished:** `551`, `621`. Record: `docs/sessions/2026-09-18-classifier.md`.
...
- **2026-09-18 `roles`** — 11 published rows shipped `class_decision=reject`, neither IN nor excluded-with-reason. Reject→withdrawn sweep (12 records, reversible) + export tripwire; a closure sentence needs the row's OWN address (0 closures, 2 counted); `_rename_record` shared, alias no-twin renames, BioCatch tie-break. 0 BD, 0 seam calls. CI pending. **NOT finished:** `624`, `625` (`623` closed same day by `classifier`). Record: `docs/sessions/2026-09-18-roles.md`.
