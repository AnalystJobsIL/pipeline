...
- **2026-08-28, 10 lanes** — `infra` (x2), `docs` (x2), `classifier` (x2), `registry`, `scraper`, `company-intel`, `jd-text`. Folded to a pointer on 2026-08-30 by `registry`, as the 2026-08-24/25/26/27 batches were and for the same reason (`docs/BACKLOG.md` 338): the word cap left no room for the day's entry. `docs/sessions/2026-08-28-*.md`.
...
- **2026-08-29 `jd-text`** - **6/0/1/0** JDs over four green digests, none paid: the cooldown parked the free rungs, the Unlocker never rendered JS, the title gate decided what to KEEP. Archive pool + `jd-archive.yml`, HiBob rung, red tests de-censused. `docs/sessions/2026-08-29-jd-text.md`. **NOT done:** 428-435.
- **2026-08-29 `registry` (queue)** - `no-board` retirements were judged from the hunt's own evidence, so they only re-confirmed it: an independent search disagreed with **15 of 20**. 120 overturned, none pruned. That instrument became `queue_resolve_search.py`: **56%** vs the nightly arm's 2-3%. **483 rows** (153 active). **NOT finished:** 508 owed, **427** - nothing schedules it. `docs/sessions/2026-08-29-registry-queue.md`.
