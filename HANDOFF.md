...
- **2026-09-16 `infra`** — the anonymous archive rung named ~9 a night. `9bae94a`: the account's SPN2 API — a job per url read to its end, `status_ext` on the line, the probe bounds workers and day, `unauthenticated` composes with `zero-produce`. Hand run: 13 jobs, 4 captured, 0 pending. 0 credits. CI `35123005923` **16/16**. **NOT finished:** `620` (re-scoped). Record: `docs/sessions/2026-09-16-infra.md`.
