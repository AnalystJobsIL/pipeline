...
- **2026-09-19 `roles`** — the first unattended reject sweep deleted **3** rows on zero live NOs: a served-stale NO counted as the run's own (Amitim, Menora), a same-run rename orphaned the backfill's accept (הפניקס). Both fixed; machine withdrawals are re-offered; the clause names all 16, not 3. Scratch proof 76→74. **NOT finished:** `648`, `649`. Record: `docs/sessions/2026-09-19-roles.md`.
