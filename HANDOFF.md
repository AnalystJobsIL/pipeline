...
- **2026-08-24 → 2026-08-31, 55 session lines, ten lanes** — folded to pointers (338, 361; `infra` (b), `docs` (c) by `scraper`; `classifier` (b) + `render` (b) 2026-08-31, both at the word cap). Records: `docs/sessions/2026-08-*.md`; `registry` (d) in `2026-08-30-infra-b.md` §0. NOT finished: 491/501/502 (`infra` b), 464/116/503/504. `docs` (c) CI `3453a2a` 33329623016 `success` 13/13; unrecorded CI 33298814000, 33299353269, 33298892195.
...
- **2026-08-31 `registry` (b)** — `514` misfiled: no overwrite (the 220 cap evicted, by design); the red was retry's OWN segment carrying `validate_empty`'s only selector, deleted nightly by `replace_own`. `_fold_empty` carries it forward; `_keep_selectors` refuses on a saturated cell. A disarmed drain was INVISIBLE — preflight refusal plus `empty_search_share`/`bd_spend` name the causes. **NOT finished:** `513`, `515`, `516`. Record: `docs/sessions/2026-08-31-registry.md`.
