...
- **2026-08-30 `docs` (CI)** - three guards I added passed on every laptop and failed on every push: `GITHUB_ACTIONS` was cleared in the git subprocess, not the pytest process. Fixed; the c-o-e number is a floor now. Run **33293548117**: `Unit guards` PASSED, job cancelled on its 10-min timeout. **NOT finished:** `442`, and `registry`'s mutants. Record: `docs/sessions/2026-08-30-docs-ci.md`.
