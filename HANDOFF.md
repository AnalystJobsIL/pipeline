...
| 2026-08-31 | company-intel | the first unattended digest after this push prints `registry backlog N (±D since 2026-08-30)` and `bulk cron: … of N to do` on `Company intel:`, no `Tel Aviv` blurb drop; `(first measurement)` on 08-31 = the `intel` stamp lost to persist (`451`) | 2026-08-30 | PARTIAL — subtype half PASS early: the 08-30 mail prints `transient: error_max_structured_output_retries` (449 live in production). Backlog-direction half `until 2026-08-31` (05:00 digest not yet run) |
| 2026-09-01 | company-intel | the first unattended digest on a headSha ⊇ the `display_name` commit logs `display_names=72 (+0/-0) divergent=55 sectors_folded=0` (±evidence drift) in step `firmo_drain` | | not yet due |
...
- **2026-08-31 `company-intel` (b)** — slugs shipped as employer names. `display_name`, evidence-only: 72 written in `--export` (an opus audit cut 32 from the first 104), 55 divergent refused, 4-row override table; sector case-folded, 565 records (482 closed). **NOT finished:** 503-504; board moves when `render` lands. Record: `docs/sessions/2026-08-30-company-intel-b.md`.
