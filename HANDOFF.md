...
| 2026-09-01 | roles | first digest on this commit: step log `alias folds: 1 superseded (NVIDIA<-NVIDIA AI) · 3 renamed` ONCE, never on 09-02; `Roles:` `retitle folds` <= 2; `Stages:` carries NO `roles seen-id collision`; `grep -ci 'senior business intelligence analyst' cloud_state/roles.csv` = **1** (2 today) | | not yet due |
| 2026-09-01 | roles | LTX flips to its own board: `grep -i "^ltx\|" cloud_state/roles.csv \| cut -d, -f6` = a `job-boards.greenhouse.io/ltx` url (the board serves the exact title, verified live). Still Indeed ⇒ the classify tier never admitted the card | | not yet due |
| 2026-09-01 | roles | public `roles.csv` header carries `description_blocker` (57 cols); meta `description_text.blocked` reads `"policy": "exclude"` (operator ruling 09-01) with per-reason counts and `reconciliation.blocked_excluded`; no published row has a blocker; the Taboola retraction (`gh_jid=8035268`) in `removed`; jd-text's `structural:*` verbatim once their driver ran | | not yet due |
...
- **2026-08-31 `company-intel` (a+b)** — a live role is never "not an employer" (`Oak` was): one `evidence_context`, and a url buys one ask about the POSTING. **28 → 4 → 0**, unmatched 3→0; `521`/`525` closed. CI **33442508283 13/13 success** (first push RED: a test read its env, and could spend). **NOT finished:** 09-01 proof, `538`. Record: `docs/sessions/2026-08-31-company-intel-b.md`.
...
- **2026-08-31 `roles` (b)** — one employer double-listed and double-judged (`533`, NVIDIA×2); 13 seen_ids named 2+ roles, one half mail-dead. Alias fold at intake + ledger (4/3 refuse); `same_role_twin` two-arm collapse; `description_blocker`, policy=exclude (operator 09-01). "~13 groups" = **3**. CI: my sha `33444708674` failure (guard: HANDOFF 3201/3200 race), master head `33445354707` **13/13 success**. **NOT finished:** `530`, `518`. Record: `docs/sessions/2026-08-31-roles-b.md`.
