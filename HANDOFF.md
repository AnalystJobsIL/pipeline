...
| 2026-09-01 | roles | public `roles.csv` header carries `description_blocker` (57 cols); meta `description_text.blocked` reads `"policy": "mark"`; every blocker sits on a `snippet`/`none` row only. jd-text's `jd_why` `structural:*` values appear VERBATIM once their driver ran | | not yet due |
