...
| 2026-09-01 | company-intel | first unattended digest on a headSha ⊇ the `display_name` commit logs `display_names=71 (+0/-0) divergent=56 sectors_folded=0` (±drift) in step `firmo_drain` | run `33739498960` | PASS with the drift this row allows: `display_names=108 (+0/-0) divergent=77 sectors_folded=0` - both exact, counts up from 71/56 over two days of intake |
...
| 2026-09-04 | classifier | **the gate reads the posting, unattended.** First `event: schedule` digest on a headSha containing this commit: `Zoll` published or named in `classify:` - the claim is that it REACHES the tier; `llm N` at most **45** above 09-03's **89**; `re-judged` under 250; `roles.csv` `grep -c` = **1** each for `doitintl`, `Investing`, `Nespresso`; Prisma `description_len` **2617** | | |
...
- **2026-09-03 `finisher`** - four defective published rows and one approved gate phrase. `_desc_appealed` reads the posting (41 cards, 0 BD, CONTRACT unmoved); Prisma repaired in BOTH stores (`jd-text`); `doit`->`doitintl` declared (`company-intel`); two url-precise retractions (`roles`). Arm B shipped over my measurement, reaffirmed. **NOT finished:** `571`-`573`. Record: `docs/sessions/2026-09-03-finisher.md`.
