...
| 2026-09-19 | roles | first `event: schedule` digest on a headSha with the reject sweep: `roles.csv` holds **0** `class_decision=reject` (was 11); `Stages:` names >=7 reasons starting `classifier `; `dataset 173 roles`; meta `excluded.withdrawn` **74** (simulated: 7 hand + 7 machine) | | |
| 2026-09-19 | roles | same digest: `Roles:` carries `closure text on 2 board-listed row(s) (ignored)`, and no HiBob/Meta closure — 0 is correct | | |
| 2026-09-19 | roles | `Roles:` carries `twin folds 1`; `roles.csv` holds ONE BioCatch BI-developer row, keyed on the plain title (the `senior` one supersedes); `ledger N = store N` | | |
| 2026-09-19 | roles | **no** `alias fold left` and **no** `alias folds: N renamed` line: the no-twin rename is measured empty and stays so until a pair carries both declarations. `de531b1` parked BOTH Harel rows; a rename line is a fold nobody declared | | |
...
- **2026-09-18 `roles`** — 11 published rows shipped `class_decision=reject`, neither IN nor excluded-with-reason. Reject→withdrawn sweep (12 records, reversible) + export tripwire; a closure sentence needs the row's OWN address (0 closures, 2 counted); `_rename_record` shared, alias no-twin renames, BioCatch tie-break. 0 BD, 0 seam calls. CI pending. **NOT finished:** `627`, `628` (`626` closed same day by `classifier`). Record: `docs/sessions/2026-09-18-roles.md`.
