...
| 2026-09-12 | company-intel | **the fold, the label and the gauge, unattended.** First `event: schedule` digest ⊇ this commit: `Company intel:` reads `digest drain (20m):` with `1 held (mars-antennas-and-rf)`; `grep -c '"DT"\|"Port.io"' cloud_state/firmographics.json` = **0** after it AND after the 14:2x cron; `registry backlog` <= **2**; one AutoDS card, reading `AutoDS` |  |  |
| 2026-09-12 | company-intel | **the render and dataset joins.** Same digest: no `blurb-names-other ... Menora` on `Render:`; `roles.csv` gives `Cal (Israel Credit Cards)` `firmo_match=exact` and a `sector`, and `Roles:` reads `firmo 0 of N unmatched` (1 of 177 = the pre-hook lag, `run.py:919`) |  |  |
...
| 2026-09-12 | ats-fetch | **the freshness verdict and the nine conversions produce, unattended.** First `event: schedule` digest on a headSha containing this commit: `Boards standing:` carries **no** `abandoned board` clause (19 were drained; the one left, Tonkean, is SPARED by `_hosted_board_alive`, so a clause here means the probe stopped answering or a tenant aged in — read the name); `Failed companies:` names no `BoardAbandoned:`; **no** `mass verdict:` line; and the nine `successfactors` rows produce — `grep -c 'careers.ey.com' docs/index.html` and the run's own `jobs_fetched`, with EY/Alstom/Ormat between them contributing >= 100 postings where the scrape read 34. `python registry_health.py --stale-boards` on that tree reads `REFUSED 0` | | |
...
- **2026-09-11 `company-intel`** — the gauge read 10 and the same ten names were in it, each held on another company's board and re-asked the identical question weekly. Name-only second ask, `_same_company_loose`, `fold_aliases` (1,607 -> 1,598), the drain label (`474`). Queue **10 -> 1**; 12 seam calls, 0 BD. **NOT finished:** 595, 596, 597. Record: `docs/sessions/2026-09-11-company-intel.md`.
