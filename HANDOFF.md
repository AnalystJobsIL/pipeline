...
|---|---|---|---|---|
| 2026-09-16 | infra | **the archive step tells "down" from "blocked", and lands captures.** First `event: schedule` `jd-archive` run ⊇ `996031e`: its `[wayback]` line has `captured` > 0, no `zero-produce`, `boards` > 0 of due, and no `host parked` (≤ 1 allowed). A line with `captured 0` and every failure `net`/`server` is **FAIL — archive down**: quote `net N, server M`, do not tune | | |
| 2026-09-17 | infra | **the backlog turns.** The 09-17 `jd-archive` schedule run's `[wayback] backlog` is below 09-16's and `boards` > 0 again. A second `stopped archive-down` / `captured 0` night is **FAIL — archive down two nights**, which is the honest reading, not a fix (`620` is the capacity question) | | |
