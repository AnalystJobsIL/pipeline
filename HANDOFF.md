...
- **2026-08-30, seventeen session lines, ten lanes** — `registry` (x4), `docs` (x2), `infra` (x2), `roles` (x2), `jd-text`, `company-intel`, `classifier`, `discovery` (x2), `render`. Folded to a pointer by `registry` (c), `roles` and `infra` (b) (338). CI runs the folded lines cited and no record holds: 33298814000, 33299353269, 33298892195. `registry` (d), which had no record, is moved verbatim into `docs/sessions/2026-08-30-infra-b.md` §0. `docs/sessions/2026-08-30-*.md`.
- **2026-08-30 `infra` (b)** — `mutation-gate (0)` killed at 40 min on every push (M1 is ONE class); split by RECORD, 5 shards, verdicts printed as they land. Drain: ingest its own `always()` step, retire-settled first, cap 350 / budgets 327. Commit gate `--strict` refuses twins and off-host rows. **NOT finished:** 491 item 3, 501, 502. Record: `docs/sessions/2026-08-30-infra-b.md`.
