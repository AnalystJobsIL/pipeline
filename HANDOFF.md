...
- **2026-09-02 `roles`** - a retraction bound to a LIVE role through another posting's stray `seen_id` (`545`), and a fold the ledger had recorded re-collided in `merged` every morning. An address a record CLAIMS now outranks one that stuck to it; a resolved twin group drops instead of skipping. Both inherited reds closed: `558@registry`, `564@jd-text`. **NOT finished:** `555`-`557`. Record: `docs/sessions/2026-09-02-roles.md`.
