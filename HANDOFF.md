...
- **2026-08-30 `docs` (CI)** - three guards I added passed on every laptop and failed on every push: they cleared `GITHUB_ACTIONS` from the git subprocess, never from the pytest process the check runs in. Fixed, and CI now verifies the reason they skip there. `coe_ratio` is a floor, not a pin. **NOT finished:** the mutation-gate reds are `registry`'s. Record: `docs/sessions/2026-08-30-docs-ci.md`.
