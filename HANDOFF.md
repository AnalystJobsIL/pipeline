...
- **2026-08-31 `registry`** — `owed` said 546 and meant 172: OWED is the drain's own selection, split from on-cadence and answered-on-disk; `queue` and `intel` reach the mail at last. Twin guard on five activation writers; `identity_ok` no longer vouches for every page (`510`); the drain had no `out/` dir. **NOT finished:** `459` (rename needs company-intel), `511`. Record: `docs/sessions/2026-08-30-registry-e.md`.
