...
| 2026-09-02 | infra | **eight shards, all under the wall.** First `tests.yml` at or after this commit: `mutation-gate (0)`..`(7)` `success`, **16 jobs** (was 13), every `timing    wall` under **1,800 s**, no `::warning::mutation shard walled` | 2026-09-01 | PARTIAL, early - the wall clause PASSES: run `33522769201`, 8 shards: **1126/1183/1390/1425/1516/1538/1600/1611 s**, all under 1,800 (mean 1,424 vs a 1,520 projection), baselines 94-112 s, no warning, 16 jobs. The `all success` clause FAILS for an unrelated reason: 5 shards report SURVIVING records from `a962ec2` (roles) and `5fb7f6d` (jd-text), reproduced at the pre-push commit - not wall time, not mine (`564`) |
...
- **2026-09-01 `roles`** — the ruling reached the dataset only, and only structural blockers: board and mail published two pending weak rows and burned both in `sent`. One judge now gates csv+board+mail. Nestlé/אסם sat in NO evidence bucket. **157→154, every row `jd`.** CI 33520122372 10/13: 4 of my own mutations SURVIVED, repaired. **NOT finished:** `555`–`557`. Record: `docs/sessions/2026-09-01-roles.md`.
...
- **2026-09-01 `infra`** — the jd-quality judge 401'd three mornings: its step carried
  ...
  finished:** the 09-02 rows prove the rest; `560`-`562`, and **`563`** (6 mutants of
  `roles`/`jd-text` survive; my 8 shards are what reported them). CI `33522769201` **10 of
  16** - 8 shards **1126-1611 s**, all under the wall; every red inherited.
