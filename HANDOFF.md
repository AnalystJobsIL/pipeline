...
- **2026-08-24 `registry`:** two activating pools could re-activate an `alias-of` duplicate and three activation branches overwrote the notes cell; the Sunday audit`'s only search rung was SerpApi (live: 0 left) so it had been a silent no-op; dry runs caught `repair_dead_urls` about to move DiA Imaging Analytics to `dia.mil` and `crack_walled` about to move Bancor to The Bancorp Bank. All fixed + guarded; new read-only `registry_health.py` (census/row-deletion guard, computed ownership matrix, ladder probe, ATS build queue). NOT done: its alarms still do not reach the daily mail — `docs/BACKLOG.md` item 3 has the exact `run.py`/`digest.py` patch. Full write-up: `docs/sessions/2026-08-24-registry.md`.
- **2026-08-24 `registry` wave 2:** two adversarial agents found nine real defects in the wave-1 commit, two CRITICAL and both in its own fixes: the crack identity gate persisted the proven-foreign URL into `api_url` with a `host documented` note, so `listing_hunt`'s fast-path activated Bancor onto The Bancorp Bank's board 24h later; and `repair_dead_urls` was hardened only on the 403 branch, so the DiA/`dia.mil` class (a `match` on a stripped core - 125 of 516 such rows) was refused only by that host's bot wall. Also: both new time budgets starved a fixed tail (no rotation - 211/211 rows never advanced), `pools()` was a sixth copy of the pool definitions and already wrong by 28 rows, the census guard filed 45 rows (7 active) as 'explained' on a substring match, and the proposed mail hook would have printed two false `rung DOWN` lines daily. All fixed + guarded; 170 tests green. NOT done: `docs/BACKLOG.md` 9-14 - the `company_identity.verdict()` ATS-subdomain hole is the root cause of two of these and is shared plumbing. Full write-up: `docs/sessions/2026-08-24-registry.md`.
- **2026-08-24 `registry` wave 3:** three independent verdict agents all returned NO-GO and all named the same defect first - `registry_health.py` exited 1 with a NameError on its default AND `--census` paths, i.e. the command ARCHITECTURE section 2 prescribes and the one BACKLOG item 12 tells infra to put in `daily-digest.yml` right before the step that saves the email. Fixed, plus: `novrfy` was still persisting an identity-UNTESTED url with a `host documented` stamp on the 0-IL branch (SupPlant was already live on careers.workable.com); `repair_dead_urls` accepted a blanket `verdict=='ats'` with no page read (tenant must now be near-equal, not merely contained); and my own `notours` note was 74 chars and evicted `unsupported ATS` from 24 of 30 crack-pool rows - shortened to parity at 17. Added a test-count FLOOR: commits 9e4ce72 and 59e222b each committed a stale `tests/test_units.py` over this lane's guards (7 then 18) and pytest stayed green both times. 175 tests. NOT done: `docs/BACKLOG.md` 9-20, all infra/shared-plumbing. Full write-up: `docs/sessions/2026-08-24-registry.md`.
