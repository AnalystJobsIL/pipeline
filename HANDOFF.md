...
| 2026-09-01 | company-intel | the first unattended digest on a headSha ⊇ the `display_name` commit logs `display_names=104 (+0/-0) divergent=43 sectors_folded=0` (±evidence drift) in step `firmo_drain` | | not yet due |
...
- **2026-08-31 `company-intel` (b)** — slugs shipped as employer names. `display_name`, evidence-only: 104 written in `--export`, 43 divergent refused, 4-row override table; sector case-folded, 565 records (482 closed). **NOT finished:** 503-504; board moves when `render` lands. Record: `docs/sessions/2026-08-30-company-intel-b.md`.
