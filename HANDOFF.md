...
- **2026-08-30 `infra` (test isolation)** — 386 measured: three registry tools lock the rung at IMPORT, not the counter; **2 of 1,496 tests reach it, 1 vacuous guard in one order**; conftest re-arms it. `tools/guard_kill.py` + CI: **184 new tests, 10 cannot fail** (`480`). **Run 33305567382: guard, guard-kill, rehearsals green; `worst/1` inherited red (registry).** **NOT finished:** 467-476, history rewrite. `docs/sessions/2026-08-30-infra.md`.
- **2026-08-30 `discovery`** — "secrethunter carries the domain" was false (seed = handle BY DESIGN). Queue 27 % noise; recruiter regex fixed. TASE/Drushim/AllJobs/RSS costed: **wire nothing**. **NOT finished:** 483-486. Record: `docs/sessions/2026-08-30-discovery.md`.
- **2026-08-30 `render`** — the H1 counted one section of a two-section mail (**6 over 13**, 6 of 10 mornings). H1 = bullets rendered + a tripwire; `same-posting` names one url under two names (both pairs: duplicate rows, not wrong employers); dataset link on the board. **NOT finished:** 487-490. Record: `docs/sessions/2026-08-30-render.md`.
