...
| 2026-09-20 | roles | **the restore fires itself.** First schedule digest ⊇ this push: `Amitim Pension Funds` and `הפניקס` 50400095 publish `class_decision=accept`; meta `excluded.withdrawn` **74** (73 if Menora returns YES); `backfill:` names ≤ 2 `a machine withdrawal owed one` | | |
| 2026-09-20 | roles | **every withdrawn role is named.** Same digest: a `roles withdrawn N role(s)` clause with N ≥ 2 lists N names, grouped `classifier N:` / `retraction line N:`, no ` +N more`. N/A if none | | |
| 2026-09-20 | roles | **green on a runner, and the mutation gate is the only verdict on my six records** (the local one was killed for memory). `gh run view 35447363465 --json jobs` (`47b3320`): every job `success` | | |
...
- **2026-09-19 `roles`** — the first unattended reject sweep deleted **3** rows on zero live NOs: a served-stale NO counted as the run's own (Amitim, Menora), a same-run rename orphaned the backfill's accept (הפניקס). Both fixed; machine withdrawals are re-offered; the clause names all 16, not 3. Scratch proof 76→74; CI `35447363465` UNVERIFIED. **NOT finished:** `648`, `649`. Record: `docs/sessions/2026-09-19-roles.md`.
