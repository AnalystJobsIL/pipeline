...
| 2026-09-12 | ats-fetch | re-read of the archived PARTIAL row: **the nine SuccessFactors conversions produce** — EY/Alstom/Ormat >= 100 postings between them where the scrape read 34 | 2026-09-13 (`ats-fetch`) | PASS - the 09-12 schedule digest (run 34685065990, `71aff56`) moved `health_baseline.json` to the 09-11 measurements: EY 9 -> **63**, Alstom 17 -> **48**, Ormat 15 -> **29** (30 on 09-13), Energean 6 -> 16, West 5 -> 7, NovoCure 5 -> 6; EY+Alstom+Ormat **140**; none of the nine in `stale.json` on 09-13 |
| 2026-09-13 | ats-fetch | re-read of the archived PARTIAL row: **the four walled boards get the rung** — `paid residential rung: ON`, `unlock_calls` > 0, EPAM/IEC/NSO/PAPAYA leave `fetch errors` or keep `http:403` WITH the unlocker counted | 2026-09-13 (`ats-fetch`) | PARTIAL - rung ON both nights: env `SCRAPE_VIA_UNLOCKER: 1` in 34666654489/34731908452 (empty in 34552848178), `unlock_calls=86`/`84`; the `ON` line exists only in the step summary (`scrape-refresh.yml:74`, `605`). 09-13: EPAM `0 (81s) unlock=4/4` -> `regressed to zero`; IEC `http:403 unlock=0/1`; NSO, PAPAYA `http:403 unlock=1/1` - the wall beats the residential IP: `registry`'s |
...
- **2026-09-13 `ats-fetch`** — 20 of the mail's 66 `regressed to zero` rows were scrapes of pages embedding a board we read. `workday_cxs_url` (public Workday site) and `fetch_teamtailor`; 10 repointed (Netafim 24 IL, Exyte 19, Arrow 5, DoubleVerify 2), 9 parked: predicted <= 49. BD 0. CI `34759619277` **15 of 16**, guard red INHERITED (`infra`'s two `bd` gauge tests, red on 640eb3c). **NOT finished:** `604`, `605`, `606`. Record: `docs/sessions/2026-09-13-ats-fetch.md`.
- **2026-09-13 `jd-text`** — `_reclean` cut 7 rows nightly and the digest's upsert re-lengthened all 7 from their cards; a closed capture lost its sentence unstamped (`587`). One `reclean_text` at four doors, cache pass (44 cards, 12:30), closure line kept and stamped, `_jd_why` named; ClixScale 94→2,737. 0 credits. CI `RUNID`. **NOT finished:** `600`a, `607`-`611`. Record: `docs/sessions/2026-09-13-jd-text.md`.
