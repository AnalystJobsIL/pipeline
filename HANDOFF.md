...
- **2026-08-30 `roles` (b)** — three PUBLIC rows were wrong (Comcast Houston, Jobgether) and nothing could remove them. Status `withdrawn` + url-keyed `roles_retractions.jsonl`, three purge sources, meta `removed` list, window **90**, `roles_archive.csv`. Store copy: **161 → 149 rows, wrong 3 → 0**. Run **33320619890** on `776e289`: guard-kill, 6 rehearsals, mutation 1+2 **success**; guard **failure inherited** (3 registry rows, fixed `a940ea4`, guard **success** on run 33321550282); mutation-gate 0 over budget (`491@infra`). **BUILT, NOT YET PUBLISHED** — the 08-31 row. **NOT finished:** 496-500, 489. Record: `docs/sessions/2026-08-30-roles-b.md`.
