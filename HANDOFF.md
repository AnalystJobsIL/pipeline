...
|---|---|---|---|---|
| 2026-09-16 | infra | **the archive step tells "down" from "blocked", and lands captures.** First `event: schedule` `jd-archive` run ⊇ `996031e`: its `[wayback]` line has `captured` > 0, no `zero-produce`, `boards` > 0 of due, and no `host parked` (≤ 1 allowed). A line with `captured 0` and every failure `net`/`server` is **FAIL — archive down**: quote `net N, server M`, do not tune | 2026-09-16 | N/A — superseded: the 09-16 schedule run had not fired by 16:20Z and `9bae94a` replaced the anonymous rung with the account's; the hand run on `9bae94a` read `captured 4, net 0, server 0, refused 9, host parked 1` (il.indeed.com on five `error:bad-request`, a real refusal). The two rows below carry the check |
| 2026-09-17 | infra | **the backlog turns.** The 09-17 `jd-archive` schedule run's `[wayback] backlog` is below 09-16's and `boards` > 0 again. A second `stopped archive-down` / `captured 0` night is **FAIL — archive down two nights**, which is the honest reading, not a fix (`620` is the capacity question) | 2026-09-16 | N/A — superseded by `9bae94a` (the authenticated rung); the 09-18 row below asks the same of the new rung's second night |
| 2026-09-17 | infra | **the account's rung lands captures unattended.** First `event: schedule` `jd-archive` run ⊇ `9bae94a`: the step log's first line is `[wayback] authenticated: N slots, …` (not `UNAUTHENTICATED`), the `[wayback]` line has `jobs` ≥ 120, `captured` ≥ 40, no `zero-produce`, `boards` > 0 of due, and 0 `pending` lines dated that day in the ledger; the after-line's `(+K this run)` equals `jobs`. `throttled` above a fifth of `jobs` ⇒ set `WAYBACK_WORKERS` to `available` − 1 (decision record §4). `wayback unauthenticated:` in the mail is **FAIL — keys not reaching the step**; `captured 0` with `net`/`server` is **FAIL — archive down** | 2026-09-16 | FAIL — run `35126457407` (schedule, `dfb1384`, 17:09Z): `authenticated: 3 slots` (no `UNAUTHENTICATED`), `jobs 56` < 120, `captured 37` < 40, `net 43` — every one `net:TimeoutError`, the POST's 30-s socket cut-off on the runner (the archive holds the POST until a slot frees; 0 of 13 on this machine), `pending 14` all with `job_id`, `boards 7`, `throttled 0`, no `zero-produce`; the after-the-day account line was absent (the re-read failed silently). Fixed the same evening (FIX_SHA: the POST waits the job's ceiling, `WAYBACK_TIMEOUT_S` 240, the re-read failure prints); the 09-18 row re-asks |
| 2026-09-18 | infra | **the POST cut-off was the leak.** First `event: schedule` `jd-archive` run ⊇ FIX_SHA: `net` ≤ 5 (of `requests`), `jobs` ≥ 100, `captured` ≥ 40, `pending` ≤ 5, the `[wayback] authenticated: … (+K this run)` line present with K == `jobs` (or its `could not be re-read` line), 14 `verified`/`error` lines from the 09-16 `pending` jobs. `net` ≥ 20 again is **FAIL — not the timeout**: quote `net`, `jobs`, `throttled` and the three `at` gaps around the `net` lines | | |
| 2026-09-19 | infra | **the backlog turns, net of inflow.** For the two schedule runs ⊇ FIX_SHA: `backlog₂ − backlog₁ ≤ (known₂ − known₁) − jobs₁`, all four from the two `[wayback]` lines (`N addresses known` is the inflow); `boards` > 0 both nights. A night that fails it with `jobs` ≥ 100 is **FAIL — the plan re-asks addresses it already holds**: quote the four numbers | | |
...
- **2026-09-16 `infra`** — the anonymous archive rung named ~9 a night. `9bae94a`: the account's SPN2 API — a job per url read to its end, `status_ext` on the line, the probe bounds workers and day, `unauthenticated` composes with `zero-produce`. First schedule run `35126457407`: 56 jobs, 37 captured, `net 43` = a 30-s POST cut-off, fixed in FIX_SHA. 0 credits. CI `35123005923` **16/16**. **NOT finished:** `620` (re-scoped); the 09-18 row. Record: `docs/sessions/2026-09-16-infra.md`.
