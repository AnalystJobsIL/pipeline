...
- **2026-08-28 `company-intel`** — the bulk cron's strikes died with its runner: `store.DEFAULT_DB` is gitignored, so a runner's store is empty every run. Ledger committed; `--export` refuses a corrupt file; backlog **139 → 4**, 137 calls, **0 BD**. **NOT finished:** 387-396. Record: `docs/sessions/2026-08-28-company-intel.md`.
