...
- **2026-08-28 `company-intel`** — the bulk cron's strikes died with its runner: `store.DEFAULT_DB` is gitignored, so a runner's store is empty every run. Ledger committed; `--export` refuses a corrupt file; backlog **139 → 7** across two registry batches, 195 calls, **0 BD**. **NOT finished:** 387-396. Record: `docs/sessions/2026-08-28-company-intel.md`.
...
- **2026-08-29 `registry` (queue)** - `no-board` retirements were judged from the hunt's own evidence, so they only re-confirmed it: an independent search disagreed with **15 of 20**. 120 overturned, none pruned. `queue_resolve_search.py` makes the scrape the board test, so an unknown ATS passes. **205 rows**, queue **877->560**. **NOT finished:** the 549-name sweep, 425, 426. Record: `docs/sessions/2026-08-29-registry-queue.md`.
