...
| 2026-08-29 | company-intel | `Company intel:` reads `export 1132+ records` with `registry backlog` in single digits, not 139, and no `::warning::company-intel`; the 08-29 `firmographics.yml` run commits `firmographics.json` **and** `firmo_failed.json` — absent by 13:00Z is 293's third data point (08-27 +605 min, 08-28 nothing) | | not yet due |
...
- **2026-08-28 `company-intel`** — the bulk cron's strikes died with its runner: `store.DEFAULT_DB` is the gitignored `state/seen.db`, so a runner's store is empty every run. Ledger committed; `--export` refuses a corrupt file and exits non-zero; backlog **138 → 4**, 137 calls, **0 BD**. **NOT finished:** 387, 388. Record: `docs/sessions/2026-08-28-company-intel.md`.
