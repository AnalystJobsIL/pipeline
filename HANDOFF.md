...
| 2026-09-12 | infra | **the ceiling is gone and the gauge is in the mail.** The 09-12 digest's `enrich` alarm carries NO `monthly-ceiling` and `matched_bd_calls` > 0; `Stage order:` carries `bd:` with a numeric `mtd=` and `projected=`; `Stages:` carries `bd projected ...` (it should: the 7-day rate is 469/day) | 2026-09-12 | PASS — `bd: 2026-09-12 (TODAY) ... mtd=6183 projected=14319` on `Stage order:`; `Stages:` carries `bd projected 14,319 credits`; the `enrich` alarm reads `scrape:bd-spent(...)` with no `monthly-ceiling`; `matched_bd_calls=6` |
...
| 2026-09-12 | company-intel | **the fold, the label and the joins ship unattended.** First `event: schedule` digest ⊇ this commit: `Company intel:` reads `digest drain (20m):` and NO `held` clause (Mars is gated to 09-18); `grep -c '"DT"\|"Port.io"' cloud_state/firmographics.json` = **0** after it AND after the 14:2x cron; `registry backlog` <= **2**; `roles.csv` gives Cal `firmo_match=exact` with a `sector` |  |  |
...
| 2026-09-12 | ats-fetch | **the freshness verdict and the nine conversions produce, unattended.** First `event: schedule` digest on a headSha containing this commit: `Boards standing:` carries **no** `abandoned board` clause (19 drained; Tonkean, the one left, is SPARED, so any clause here is a new row — read the name); `Failed companies:` names no `BoardAbandoned:`; **no** `mass verdict:` line; and the nine `successfactors` rows produce — EY/Alstom/Ormat >= 100 postings between them where the scrape read 34. `python registry_health.py --stale-boards` on that tree reads `REFUSED 0` | | |
| 2026-09-13 | ats-fetch | **the four walled boards get the rung the ceiling had switched off.** The 00:00 `scrape-refresh` runs of 09-12 and 09-13 print `paid residential rung: ON` (every run to 09-11 printed OFF under `5,804 of 5,000 credits (116%)`), and their `collect:` stamps carry `unlock_calls` > 0. Then EPAM, IEC, NSO Group and PAPAYA either leave the mail's `fetch errors` or keep their code with the unlocker counted — if they keep `http:403` WITH `unlock_calls` > 0, the wall beats the residential IP too and the class is `registry`'s, not a spend question | | |
| 2026-09-13 | infra | **the discovery step finishes, and says how.** The 09-13 digest's `discovery` step: under **15 min** (25:13 killed on 09-12, 4:12 on 09-11); its `[linkedin:*]` lines carry SPREAD timestamps, not one (all 52 shared 09:31:33 on 09-11); no `workflow step 'discovery' failure` in the mail; `discovery` in `pipeline_stages.json` dated 2026-09-13 with `linkedin_cards`>0 and `indeed_cards`>0; the `[linkedin]` line prints a blocked rate WITH its denominator and how many queries the budget cut — if it cut into the 9 NATIONAL queries the answer is the pause, not the budget. And the pacing's win must survive the bound: `source_health` linkedin >= **3,000** with `discovery_daily.py` credits <= **12** (4,888 / 6 on 09-12; 1,400-3,016 / 17-23 unpaced) | | |
| 2026-09-13 | infra | **neither JD cap binds.** The 09-13 mail carries NO `cap bound` clause of either kind; the `jd-fill:` step line has no `bd-capped`/`bd-render-capped` in `failed:` and no `discovery-indeed auth-walled` in `unfillable` (15 on 09-12, 44 on 09-10); `bd:` reads `jdfill=` > **30** (4.7 on 09-12, and 0 every day before 09-11 because nothing booked it); `classify ... judged on the title alone` < **5** and `CANNOT be re-judged` < **9**. If the mail says the TIME budget bound instead, that is the second bound in the series and the answer is `JDFILL_TIME_BUDGET_MIN`, not the cap | | |
