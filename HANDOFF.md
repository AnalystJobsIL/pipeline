...
| 2026-08-29 | company-intel | `Company intel:` reads `export 1132+ records`, `registry backlog` in single digits; **no `firmo` on `Stages:`** — the new dead-cron alarm, which must stay silent on a healthy zero-todo run. The 08-29 run commits `firmographics.json`, `firmo_failed.json` and `pipeline_stages.json`, and its `139 to do` predecessor hit `--limit 40` and left 99 — at 150 it should clear the queue | | not yet due |
...
| 2026-08-30 | jd-text | `jd-archive.yml`'s first unattended run green, summary **N filled >= 300**, **`still thin` under 1,300**; mail **`scrape_filled` > 0**, **`matched_llm_unavailable` = 0** | | |
...
- **2026-08-29 `jd-text`** - **6/0/1/0** JDs over four green digests, none paid: the cooldown parked the free rungs, the Unlocker never rendered JS, the title gate decided what to KEEP. Archive pool + `jd-archive.yml`, HiBob rung, red tests de-censused. `docs/sessions/2026-08-29-jd-text.md`. **NOT done:** 428-434.
