...
- **2026-08-31 `company-intel` (a+b)** — the queue WAS the empty string; anchors were not enough. A live role is never "not an employer" (`Oak` was): one `evidence_context` for all three callers, and a url buys one ask about the POSTING. **28 → 4 → 0**, unmatched 3→0; `521`/`525` closed; CI <PENDING>. **NOT finished:** 09-01 proof, `534`. Record: `docs/sessions/2026-08-31-company-intel-b.md`.
