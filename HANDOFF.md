...
- **2026-08-30 `render` (b)** — slug rows now prefer the firmographics `display_name` (contract agreed live with `company-intel` (b)) on every reader surface; identity guard blocks impersonation (finbounce until `487`). Inert at push (6/6 identical, KILLS 5); their field landed same evening. **NOT finished:** `504`. Record: `docs/sessions/2026-08-30-render-b.md`.
