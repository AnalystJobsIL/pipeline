...
| 2026-09-19 | roles | conditional on registry's Harel park landing first: `alias folds:` carries `1 renamed (Harel Insurance & Finance<-הראל ביטוח ופיננסים)`. `N/A` if that commit is not in the headSha | | |
...
- **2026-09-18 `roles`** — 11 published rows shipped `class_decision=reject`, neither IN nor excluded-with-reason. Reject→withdrawn sweep (12 records, reversible) + export tripwire; a closure sentence needs the row's OWN address (0 closures, 2 counted); `_rename_record` shared, alias no-twin renames, BioCatch tie-break. 0 BD, 0 seam calls. CI pending. **NOT finished:** `623`-`625`. Record: `docs/sessions/2026-09-18-roles.md`.
