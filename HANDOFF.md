...
|---|---|---|---|---|
| 2026-08-31 | scraper | first `event: schedule` collect stamp on the 08-30 scraper commits: NO `fabricated-loc` token; `loc_unknown=`/`legacy_loc=`/`ownless=` present; post-cron cache's unprovenanced bare-`Israel` count (one-liner: `docs/sessions/2026-08-30-scraper.md`) = `legacy_loc`; lakeFS and nsKnox cached | 2026-08-31 | PASS — run `33350105122` (schedule, headSha `15c7c74` ⊇ `6c1dd1c`): no `fabricated-loc`; `loc_unknown=1653 legacy_loc=17 ownless=640`; cache one-liner = **17** = `legacy_loc`; lakeFS Director + nsKnox cached; bare-Israel 700→379, Comcast/Hunter Douglas/Arm gone |
