...
| 2026-08-31 | roles | **the wrong rows are gone from the PUBLIC file without a session:** `curl -s https://analystjobsil.github.io/board/roles.csv \| grep -c 'Comcast\|Jobgether'` is **0** (was 3), the meta says `"published_on_pages": true` and lists them under `withdrawn`, and that run's mail carries `withdrawn 2` on `Roles:` and `roles withdrawn 2 role(s)` on `Stages:`. Still 3 means the publish step was skipped — check `cloud_state/roles.csv` on master first | | not yet due |
...
- **2026-08-30, thirteen records, nine lanes** - `registry` (x3, this line is the third), `docs` (x2), `infra` (x2), `jd-text`, `company-intel`, `roles`, `classifier`, `discovery`, `render`. Folded to a pointer on 2026-08-30 by `registry` (c), same reason (338). CI runs the folded lines cited and no record holds: 33298814000 (classifier), 33299353269 (registry), 33298892195 (docs-ci). `docs/sessions/2026-08-30-*.md`.
- **2026-08-30, 2 session lines** — `registry` (c), `discovery`. Folded 2026-08-30 evening by `roles` to stay inside the word cap (338); every run id and UNVERIFIED-IN-CI note is in its record. `docs/sessions/2026-08-30-*.md`.
- **2026-08-30 `roles` (b)** — three PUBLIC rows were wrong (Comcast Houston, Jobgether) and nothing could remove them. Status `withdrawn` + url-keyed `roles_retractions.jsonl`, three purge sources, meta `withdrawn` list, window **90**, `roles_archive.csv`. Store copy: **161 → 149 rows, wrong 3 → 0**. **Not yet verified LIVE** (08-31 row). **NOT finished:** 496-500. Record: `docs/sessions/2026-08-30-roles-b.md`.
- **2026-08-30 `registry` (d)** - master red on two rows the 14:06Z coverage audit (`7319f85`, run 33304997204) re-activated: `JPMorgan Chase` beside active `JPMorganChase` (parked `alias-of`), Renesas on a company-site url (repaired to the SR endpoint); `Checkout`/`finbounce` parked `duplicate of` (487@render). Nothing stops the audit recurring: `audit_empty_rows`/`crack_walled` activate without an active-twin or endpoint-shape check. Record: this line.
