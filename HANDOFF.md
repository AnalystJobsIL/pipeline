...
- **2026-09-18 `registry`** — a NOT-THEIRS ledger read bound nothing, so the hunt re-activated Kima and PayPlus. `board_verify.refuses` now vetoes both hunt arms and `--verify-existing`; `ledger_contradicted` **3→0**. Lapse-wave alarm fixed; search-llm back-off 14→28→56/90 (09-29 **225→123**). Harel board found, gate refuses (`621`). BD **0**, seam **2**; `459` declined: 3-lane rename. CI <id> **k/16**. **NOT finished:** `621`, `622`. Record: `docs/sessions/2026-09-18-registry.md`.
