...
| 2026-09-19 | jd-text | the first `event: schedule` digest whose `headSha` contains this commit: enrich `matched_recleaned` <= 3 and NO `matched:reclean-refused`; `grep -c '&#' cloud_state/roles_text.jsonl` = 0; the two `אסם` rows' `description_len` 1678 and 1388; the `classify` line carries no `? 1`; every `held` record's `jd_why` is non-empty | | |
| 2026-09-20 | jd-text | the first `event: schedule` `jd-archive` whose `headSha` contains this commit: `scrape_recleaned` <= 274 and NO `archive:reclean-refused`; the run AFTER that one <= 5 | | |
...
- **2026-09-18 `registry`** — a NOT-THEIRS ledger read bound nothing, so the hunt re-activated Kima and PayPlus. `board_verify.refuses` now vetoes both hunt arms and `--verify-existing`; `ledger_contradicted` **3→0**. Lapse alarm fixed; back-off 14→28→56/90 (09-29 **225→123**). Harel board found, gate refuses (`621`); `Trivago` `alias-of Holisto`. BD **2**, seam **2**. CI `35359064839` **1/16, queued** (09-19 rows). **NOT finished:** `621`, `622`, `459`. Record: `docs/sessions/2026-09-18-registry.md`.
...
- **2026-09-18 `roles`** — 11 published rows shipped `class_decision=reject`. Reject→withdrawn sweep (12 records, reversible) + export tripwire; a closure sentence needs the row's OWN address (0 closures, 2 counted); `_rename_record` shared, alias no-twin renames, BioCatch tie-break. 0 BD, 0 seam. CI pending. **NOT finished:** `627`, `628`. Record: `docs/sessions/2026-09-18-roles.md`.
- **2026-09-18 `scraper`** — 54 cards carried a neighbour's link, 20 a sibling's text, אסם its navbar slogan. Card bounds, own-address carry key, h1-anchored place, `608`(a); replays: 447 boards 0 lost / 43 addresses corrected, 452 pages 35 places moved, 10 cards voided. BD 2. CI `PENDING`. **NOT finished:** `616`, `623`-`625`. Record: `docs/sessions/2026-09-18-scraper.md`.
- **2026-09-18 `jd-text`** — own-board nav bracketed two published texts (62-70% chrome), 26 rows carried raw entities, a 573-char Google card passed the bar, held rows named no reason. Mirror rule, 3 markers, unescape, listing-card veto, `failed:` reasons, defective-text arm; 46 re-cleaned, entities 26→0, fixed point proved, 2 credits. CI RUNID. **NOT finished:** 607, 608, 629, 630. Record: `docs/sessions/2026-09-18-jd-text.md`.
