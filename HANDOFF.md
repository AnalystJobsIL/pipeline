...
- **2026-09-13 `company-intel`** — `saver1` was ours (SaverOne, the page's spelling), and three records bought off foreign boards still answered for their names. `ALIASES` declaration; dated `DISOWNED` drop via one `settle_keys` at every view; searchless answers named (`597` CLOSED). Render set **1 → 0**; export 1,651 → 1,649; 1 seam call, 0 BD. CI PENDING. **NOT finished:** `618`. Record: `docs/sessions/2026-09-13-company-intel.md`.
