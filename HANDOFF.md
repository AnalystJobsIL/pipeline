...
| 2026-09-01 | jd-text | **the donor rung survives a run nobody watched.** Stamp carries `matched_via_sibling`/`matched_sibling_refused` (absent ⇒ it never ran), `matched_structural` **= 1**, no `donor-all-refused`; `jd_why like 'structural:%'` is **exactly Taboola**; `Indeed cap bound at 25` absent or **< 20** | | |
...
| 2026-09-01 | roles | first digest on this commit: step log `alias folds: 1 superseded (NVIDIA<-NVIDIA AI) · 3 renamed` ONCE, never on 09-02; `Roles:` `retitle folds` <= 2 (1 = the sweep took Fetcherr, also fine); `Stages:` carries NO `roles seen-id collision`; `grep -ci 'senior business intelligence analyst' cloud_state/roles.csv` = **1** (2 today) | | not yet due |
| 2026-09-01 | roles | LTX flips to its own board: `grep -i "^ltx\|" cloud_state/roles.csv \| cut -d, -f6` = a `job-boards.greenhouse.io/ltx` url (today `il.indeed.com/viewjob?jk=b17171f8cdb4b6c7`; the greenhouse board serves the exact title, verified live). Still Indeed ⇒ the classify tier never admitted the card — not the ratchet | | not yet due |
| 2026-09-01 | roles | public `roles.csv` header carries `description_blocker` (57 cols); meta `description_text.blocked` reads `"policy": "exclude"` (operator ruling 09-01) with per-reason counts and `reconciliation.blocked_excluded`; NO published row has `description_quality` snippet/none WITH a blocker; the Taboola retraction (`gh_jid=8035268`) is in `removed`; jd-text's `structural:*` verbatim once their driver ran | | not yet due |
...
- **2026-08-24 → 2026-08-31, 58 session lines, ten lanes** — folded to pointers, all at the word cap (`registry` 08-31 by `company-intel`: OWED 546→1, `510`, CI 33366050922 13/13; `classifier` 08-31: `class_decision` 33→0, 14 withdrawn, `v3.7cb6831f`, CI `70fb985` 33412050175 13/13; 338, 361). Records: `docs/sessions/2026-08-*.md`; `registry` (d) in `2026-08-30-infra-b.md` §0. NOT finished: 491/501/502, 464/116/503/504, 505-508, 459/511, 500@roles, 529-532.
...
- **2026-08-31 `jd-text` (a+b)** — (a) Indeed's pane fills 90 of 92, 104 credits; (b) the donor rung (own-address·cache·copy·archive) fills a role from its own other copies, admitted only on the document naming this role. **Matched failing `looks_like_jd` 7 → 1**, 4 free + 2 credits; `jd_why` closes `443`'s half; cap 8→25. **NOT finished:** `534`-`537`. Record: `docs/sessions/2026-08-31-jd-text-b.md`.
- **2026-08-31 `company-intel` (a+b)** — a live role is never "not an employer" (`Oak` was): one `evidence_context`, and a url buys one ask about the POSTING. **28 → 4 → 0**, unmatched 3→0; `521`/`525` closed. CI: first push RED (a test read its env, and could spend), fixed — **33442508283 guard+guard-kill success**. **NOT finished:** 09-01 proof, `538`. Record: `docs/sessions/2026-08-31-company-intel-b.md`.
- **2026-08-31 `registry` (b)** — `514` misfiled: the red was retry's OWN segment carrying `validate_empty`'s only selector. `_fold_empty` carries it forward; `_keep_selectors` writes only what costs no pool and keeps the row retirable (the first fold blocked `domain-dead` on 6/26). Disarmed drains were INVISIBLE: 4-condition preflight, per-night `empty_search_share`. Intel 22→18. CI **33418948906 13/13**. **NOT finished:** `513`, `516`-`520`. Record: `docs/sessions/2026-08-31-registry.md`.
- **2026-08-31 `roles` (b)** — one employer double-listed and double-judged (`533`, NVIDIA×2); 13 seen_ids named 2+ roles, one half mail-dead. Alias fold at intake + ledger (4/3 refuse, evidence-gated); `same_role_twin` two-arm collapse; `description_blocker` + jd_why carry (jd-text contract live). "~13 groups" re-derived = **3**. **NOT finished:** `530`, `518`, Fetcherr arm-dependent. Record: `docs/sessions/2026-08-31-roles-b.md`.
