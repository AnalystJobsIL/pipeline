...
|---|---|---|---|---|
| 2026-09-16 | infra | **the archive step tells "down" from "blocked", and lands captures.** First `event: schedule` `jd-archive` run ⊇ `654db51`: its `[wayback]` line has `captured` > 0, no `zero-produce`, `boards` > 0 of due, and no `host parked` (≤ 1 allowed). A line with `captured 0` and every failure `net`/`server` is **FAIL — archive down**: quote `net N, server M`, do not tune | | |
