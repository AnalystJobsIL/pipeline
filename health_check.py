...
def _cache_at(rev):
    ...
    EXTRACTED cards, so when the extractor changes there is nothing to replay. `{}` for a
    revision or file that cannot be read, AND for valid JSON of the wrong shape (a `[...]`
    cache would otherwise crash the report two lines later) — an empty report, never a
    crash, which is the same rule `health._load` follows."""
    def _shaped(text):
        try:
            d = json.loads(text)
        except ValueError:
    ...
    if os.path.exists(rev):
        try:
            with open(rev, encoding="utf-8") as f:
                return _shaped(f.read())
        except OSError:
            return {}
    try:
        out = subprocess.run(["git", "show", f"{rev}:scraped_cache.json"],
                             capture_output=True, timeout=60)
        return _shaped(out.stdout.decode("utf-8", "replace")) if out.returncode == 0 else {}
...
def rebase_report(rev, stale_path=health.STALE, baseline_path=health.BASELINE, out=print):
    ...
    for name, v in rows.items():
        ...
        posts = [p for p in cached if isinstance(p, dict)] if isinstance(cached, list) else []
        reported[name] = posts
...
def _operand(argv, flag):
    """The value after `flag`, or None when it was given last — `argv[i + 1]` raised
    IndexError for `--rebase-scrape` with no revision, after printing a whole report."""
    i = argv.index(flag)
...
def _rebase_cli(argv):
    ...
    Non-zero when the correction could not be written: an operator tool that says `[XX]` and
    exits 0 is the "green run that proves nothing" this repo is arranged against."""
    rev = _operand(argv, "--rebase-scrape")
    if not rev:
        print('usage: health_check.py --rebase-scrape <rev|path> [--apply "Name,Name"]')
        return 2
    rebase_report(rev)
    if "--apply" not in argv:
        print("\n(report only; re-run with --apply \"Name,Name\" to zero those baselines)")
        return 0
    raw = _operand(argv, "--apply")
    if raw is None:
        print('usage: --apply "Name,Name"  (comma-separated, quoted)')
        return 2
    ...
    return 1 if "<write>" in result["refused"] else 0
