...
|---|---|---|---|
| **`discovery`** | 1 | **6 of the 471 names in the intake queue carry a conclusive retirement and were re-added anyway** (re-derived 2026-09-03; it was **165 of 572** on 2026-08-30). **The fall is the 08-30 day-cap and `RETIRED_VERDICTS` skip running nightly, not a session's hand-drain** — the 2026-09-03 `discovery` session did NOT move this number and says so. Nothing that writes `research_companies.json` reads `cloud_state/queue_disposition.json`, so the queue cannot stay drained (`441`). Intake is throttled since today — the catalog offer is capped at **40 a DAY** (`SECRETHUNTER_DAY_CAP`, `pipeline/secrethunter.py`; steady state ~31; first unattended proof is a `[secrethunter]` step line reading `day window 40`) — and the queue is **27 %** noise on a seeded 100 (foreign, not-an-org, recruiters the gate passes; `484`). Target 0 re-added. `python -c "import json;d=json.load(open('cloud_state/queue_disposition.json',encoding='utf-8'));r=json.load(open('research_companies.json',encoding='utf-8'));print(sum(1 for x in r if (x.get('name') or '').strip().lower() in {k.strip().lower() for k,v in d.items() if (v.get('verdict') or '') in ('no-board','duplicate-of','not-an-employer','acquired-by')}))"` — *(owns: where new roles and new employers come from)* | `discovery_daily.py`, `discovery_telegram.py`, `pipeline/aggregators.py`, `pipeline/recruiters.py` |
| **`registry`** *(one at a time)* | 2 | **37 OWED of a 414-name unsettled queue** (2026-09-01 15:0xZ, `python queue_state.py`: 37 OWED · 360 ON CADENCE, answered and waiting out 14 days · 17 answerable from disk; all 37 sit on the cheapest rung, `own-site`). It read 65 on 2026-08-31 and the first real drain has run since. **Read `OWED`, not the queue size, and not the mail's `owed=`** — the 08-31 digest stamped `owed=369` at 11:29Z and the same tree read 65 by 14:20Z, because that run's dispose step retires names AFTER composing its own stamp. The number under this one is capacity: the nightly drain does **176** since 2026-09-13 (112 before; `queue_resolve_search.nightly_capacity()`) against brand-new intake of **161 a day at the median, 212 mean** (seven days to 08-30), and on 2026-09-13 the selectable set read **190** (35 new, 155 lapsed repeats). A drain below intake cannot hold, whatever a session does by hand (`491@infra`, retire-settled first). **The drain has never once recorded an attempt in the cloud** — every previous night died in 6 seconds on a missing `out/` dir under `continue-on-error`; fixed (`df803a1`), first real test is the 08-31 19:00 run, and a disarmed key can no longer masquerade as one (`empty_search_share`, `queue drain BOUGHT NOTHING`). Target 0, and a drain that exceeds intake. `python queue_state.py` (read-only without `--ingest`) — *(owns: dark rows, and the 5-rung resolution ladder (§3) over 9 re-check pools)* | **`registry_health.py`** (read-only: census, who re-checks what, which ATS to build, `--explain "<name>"`), `companies.csv`, `listing_hunt.py`, `triage_dark.py`, `crack_walled.py`, `deep_validate.py`, `repair_*.py`, `resolve_*.py`, `audit_empty_rows.py`, `probe_candidates.py`, `scan_dead_domains.py`, `auto_expand.py`, `apply_resolved.py`, `confirm_zero.py` (the zero audit: it parks ACTIVE rows `needs re-resolution`, so it writes registry verdicts), `queue_disposition.py` (the only sanctioned way a NAME leaves `research_companies.json` — it writes the evidence; `discovery` owns the re-add that ignores it, `441`) |
| **`ats-fetch`** | 3 | **0 active rows REFUSED as abandoned** (re-derived 2026-09-11; it read 18 on 2026-08-30 and **19** that morning). A board that answers and has not moved since 2024 is a row we count as covered and a company we are not covering, and since 2026-09-11 the pipeline says so itself: `fetchers.fetch_company` raises `BoardAbandoned`, the mail names it, the self-heal re-resolves it. **Read REFUSED, not the headline count** — the tool prints `ABANDONED … 1 - of which SPARED by a live hosted board 1 = REFUSED 0`, and the 1 is `Tonkean`, whose Lever board answers while Lever never bumps `createdAt`. The 19 were drained by hand the day the verdict landed (11 re-pointed, 15 parked), and **nothing parks an abandoned row without a session** (`593`), so this number rises again the first time a tenant goes quiet and the self-heal cannot repair it. Target 0 REFUSED. `python registry_health.py --stale-boards` — *(owns: how a board's API is read; adding a platform)* | `pipeline/fetchers.py`, `pipeline/platform_check.py`, `pipeline/health.py` |
...
| **`jd-text`** | 4 | **10 of 248 matched roles fail `looks_like_jd`** (re-derived 2026-09-11 after the chrome class was cut; it read 3 of 193 on 09-01, and the pool has grown). Every one is honest and named in `docs/sessions/2026-09-11-jd-text.md`: two page-slices, two rows whose own page serves no posting, three aggregator-only rows, `taboola|product analyst maternity…` (no text in any store at any commit — ARCHITECTURE §5), `mizrahi…|analyst` (a real 1,266-character Hebrew posting with one marker family; the Hebrew twin marker that would rescue it promotes four site-navigation pages, so it was REJECTED) and `הפניקס|דאטה אנליסט ית`. **The second number is the one to read, and it ROSE on purpose: 74 of 238 rows that pass the bar carry no strict mention of their own employer** (`quality_suspect` → `no-company-echo`), against 22 before — the LinkedIn chrome used to repeat the company name half a dozen times, so a posting that never names its employer looked as though it did. Each awaits one cached model call, and the tier is still auth-refused on the runner (`llm-auth13`, `infra`'s Open item 3), so guards 3 and 4 of §7a only bite when a session runs the driver by hand. **109 attempted scrape cards are still under 200 characters** (of 1,027 attempted), and the scrape cache is not re-cleaned at all (`581`). The MATCHED per-row reason IS committed state (`matched.jd_why`); the scrape-card reason still is not. `python -c "import sqlite3,sys;sys.path.insert(0,'.');from pipeline.jdfill import looks_like_jd,quality_suspect;c=sqlite3.connect('cloud_state/seen.db');r=[x for x in c.execute('select mkey,coalesce(company,\"\"),coalesce(description,\"\"),coalesce(jd_why,\"\") from matched where coalesce(status,\"\")!=\"superseded\"')];print(len([1 for m,co,d,w in r if not looks_like_jd(d)]),'no-JD;',len([1 for m,co,d,w in r if looks_like_jd(d) and quality_suspect(d,company=co)=='no-company-echo']),'no-echo')"` — *(owns: every relevant role gets its description, whatever its age — and that it is THAT role's)* | `pipeline/jdfill.py`, `enrich_scrape_jd.py`, `enrich_matched_jd.py` |
| **`company-intel`** | 4 | **1 active row with no sector, by `identity_key`** — `Discovery` (the aggregator pseudo-row, which is not an employer and never will be). Re-derived 2026-09-13 evening after `Saver1` landed (`Mars Antennas And Rf Systems` left the count when `registry` parked it), down from 21 on 08-30 and 11 on 09-11. **The two gauges here are different universes and always were**: the command in this cell counts ACTIVE ROWS and includes the `discovery` pseudo-row, while the mail's `registry backlog` counts the RENDER SET (active rows ∪ every company with a role record, minus the pseudo-row and minus `not_a_company` names) and read **0** on 2026-09-13 evening; at 05:00 it is never below the names that morning's drain could not answer (`ARCHITECTURE.md` §7). Quote which one you mean. The number is a CADENCE before it is a capacity — every cron that adds a row moves it and no push can, which is why `tests/test_company_intel.py` measures and prints it rather than capping it. Target 0. `python -c "import json,csv;from pipeline.firmographics import identity_key as k;f=json.load(open('cloud_state/firmographics.json',encoding='utf-8'));s={k(n) for n,v in f.items() if (v.get('sector') or '').strip()};print(sum(1 for r in csv.DictReader(open('companies.csv',encoding='utf-8')) if r['active'].strip().lower()=='true' and k(r['company_name']) not in s))"` — *(owns: sector / stage / employees / founded / Israel centre)* | `pipeline/firmographics.py`, `pipeline/company_info.py`, `research_firmographics.py`, `bd_employees.py`, `fill_employees_llm.py`, `company_type_analysis.py` |
| **`classifier`** | 5 | **0 role records without a classifier verdict**, and it held through the 2026-09-01 digest — the 08-31 backfill's 33 went to 0 and stayed there (`backfill: 0 verdict-less record(s)` in run 33494404810). Re-derive: python -c "import json;print(sum(1 for l in open('cloud_state/roles.jsonl',encoding='utf-8') if l.strip() and not (json.loads(l).get('class') or {}).get('decision') and json.loads(l).get('status') in ('open','closed')))". **The number under it is the contract drain**, and the command that reads it was WRONG until 2026-09-01: it took `max()` over the contract PREFIX, which is a hex hash, so a job re-judged under a contract that sorts low counted as superseded — it read 537 of 561 on a morning whose true figure was **189 of 561** (`541@classifier`, the same bug in production's own `_lookup`, fixed). Today, after the 2026-09-01 boundary bump (`v3.7cb6831f` → `v3.0f84ab84`) and the 2026-09-14 data-platform ruling's bump (`v3.0f84ab84` → `v3.0a439b16`, landed 2026-09-18), it is **561 of 561** by construction and the drain starts from a full pool: the 09-01 run re-judged 339 (242 NO + 97 stale-YES, `flipped +5/-8`) and had reached 372 of 561 current before the bump re-superseded them. That is the price of a scope change, stated where the number is so nobody subtracts the two. Expect several mornings, not one — the drain is **encounter-limited, not cap-limited** (it only reaches a verdict when that posting appears in the run; no cap bound on 09-01 either) — plus **13 that no cap reaches**, having no description (`464`, lane `jd-text`). python -c "import sqlite3;from pipeline import seniority as s;c=sqlite3.connect('file:cloud_state/seen.db?mode=ro',uri=True);j={};[j.setdefault((s._versioned(k)[0].rsplit('|',1)[0] if s._versioned(k) else k),set()).add(s._versioned(k)[1] if s._versioned(k) else 'legacy') for k,_ in c.execute('select title_key,verdict from llm_cache') if not k.startswith('jdq1|')];print(sum(1 for v in j.values() if s.CONTRACT not in v),'of',len(j))" — *(owns: which roles qualify, and the LLM tier that decides the ambiguous ones)* | `pipeline/seniority.py`, `pipeline/class_backfill.py`, `pipeline/israel.py`, the `llm_cache` key scheme; `pipeline/llm.py` is shared |
| **`roles`** | 6 | **9 published rows the record's own evidence contradicts, and 38 whose verdict has no provenance** (2026-09-11). The 09-02 reading this replaces — 1 adjudicated-OUT row and 2 seen-id collisions — is **0 and 0** today, answered by the 09-03 morning check, so that class closed. What is left is three classes, each measured on `cloud_state/roles.jsonl` (262 records) and each fixed in code today but **not yet proven by an unattended run** — until the 2026-09-12 rows answer, this reads 4, 5 and 38, not 0, 0 and 38: **4** open rows whose own stored page says the posting stopped accepting applications (`586`, LinkedIn-only, the marker in LinkedIn's chrome; `migdal|business analyst` was on the board 17 days and emailed); **5** rows keyed on a card blob rather than a title (`580`, `We're Hiring … - Practical Vision` published AND emailed); and **38** `closed`+`accept` records whose verdict is frozen under whatever contract judged it on the day it closed (`544` — the new `class_contract` column makes the freeze legible, and nothing DRAINS it, which is `classifier`'s `judge_backfill` reaching only empty cells). Target 0, 0, and falling. `python -c "import json,sys;sys.path.insert(0,'.');from pipeline import roles as R;t={};[t.update({json.loads(l)['role_id']:json.loads(l).get('description') or ''}) for l in open('cloud_state/roles_text.jsonl',encoding='utf-8') if l.strip()];r=[json.loads(l) for l in open('cloud_state/roles.jsonl',encoding='utf-8') if l.strip()];print(sum(1 for x in r if x.get('status')=='open' and R.page_closed(dict(x,description=t.get(x['role_id'],'')),x)),'page-closed;',sum(1 for x in r if x.get('status') not in ('superseded','purged','withdrawn') and R.canonical_title(x.get('title'),x.get('company'),x.get('location'))!=(x.get('title') or '')),'blob;',sum(1 for x in r if x.get('status')=='closed' and (x.get('class') or {}).get('decision')=='accept'),'frozen')"` — *(owns: the role as an ENTITY: is it the same one, is it still open, was it re-posted, when does it leave the board)* | **`pipeline/roles.py`** (the ledger: `cloud_state/roles.jsonl` + `roles_text.jsonl`), `pipeline/store.py` (`matched`/`sent`, `merge_key`, `seen_id`, `merge_duplicates`, `filter_new`, `upsert_matched`, `rekey_matched`), the role-selection block in `pipeline/run.py`, repost detection |
