...
| **`jd-text`** | 4 | **1 of 187 matched roles fails `looks_like_jd`** (2026-08-31 evening, was 9 of 172), and it is the one that CANNOT be filled: `taboola\|product analyst maternity leave replacement`, 404 at its own Greenhouse board with no copy ever held, carrying `jd_why = structural:gone(donors:0)`. The matched half is the half that matters — those are the roles the classifier judges and the board shows — and the number to read is now `matched.jd_why`, not a character count: a row is finished when it has its text OR a written structural reason. **90 attempted scrape cards are still under 200 characters** (of 971 attempted; the 204-of-1,404 reading it replaces was taken before the `scraper` lane rebuilt that cache, so most of that fall is theirs, not this lane's). Target 0 unexplained on both halves. The MATCHED per-row reason IS committed state since 2026-08-31 (`matched.jd_why`, `443`'s jd-text half); the scrape-card reason still is not, and the `~30 a night that fail their fill` figure remains a RUN counter. `python -c "import sqlite3;from pipeline.jdfill import looks_like_jd;c=sqlite3.connect('cloud_state/seen.db');print([(m,w) for m,d,w in c.execute('select mkey,coalesce(description,\"\"),coalesce(jd_why,\"\") from matched where coalesce(status,\"\")!=\"superseded\"') if not looks_like_jd(d)])"` — *(owns: every relevant role gets its description, whatever its age)* | `pipeline/jdfill.py`, `enrich_scrape_jd.py`, `enrich_matched_jd.py` |
...
| **`classifier`** | 5 | **0 role records without a classifier verdict** — 33 of the 167 published rows carried an empty `class_decision` on the morning of 2026-08-31, every one `closed`, and nothing could reach them: `rec["class"]` has ONE writer, fed only from a run's own `merged` jobs, and the contract drain re-judges RECORDED verdicts. `pipeline/class_backfill.py` now judges them, from `run.py` every morning. Target 0 in the ledger: python -c "import json;print(sum(1 for l in open('cloud_state/roles.jsonl',encoding='utf-8') if l.strip() and not (json.loads(l).get('class') or {}).get('decision') and json.loads(l).get('status') in ('open','closed')))". The published CSV still shows 33 until the next digest regenerates it (then 0 of ~153): python -c "import csv;print(sum(1 for r in csv.DictReader(open('cloud_state/roles.csv',encoding='utf-8')) if not r['class_decision']))". Behind it, the drain: the 08-31 scope change superseded **541 of 541** cached verdicts (`v3.7cb6831f`), re-bought at 250 NO + 150 YES a run behind an 80-call fresh reserve. Expect several mornings, not one — the drain is **encounter-limited, not cap-limited** (run 33387229779 re-judged 150 of a forecast 210 with no cap binding; it only reaches a verdict when that posting appears in the run) — plus **12 that no cap reaches**, having no description (`464`, lane `jd-text`). python -c "import sqlite3;from pipeline import seniority as s;c=sqlite3.connect('file:cloud_state/seen.db?mode=ro',uri=True);b={};rows=[(k,s._versioned(k)) for k,_ in c.execute('select title_key,verdict from llm_cache') if not k.startswith('jdq1|')];[b.__setitem__(sp[0].rsplit('|',1)[0] if sp else k, max(b.get(sp[0].rsplit('|',1)[0] if sp else k,''), sp[1] if sp else 'legacy')) for k,sp in rows];print(sum(1 for x in b.values() if x!=s.CONTRACT),'of',len(b))" — *(owns: which roles qualify, and the LLM tier that decides the ambiguous ones)* | `pipeline/seniority.py`, `pipeline/class_backfill.py`, `pipeline/israel.py`, the `llm_cache` key scheme; `pipeline/llm.py` is shared |
| **`roles`** | 6 | **3 wrong rows in the PUBLIC csv** (`cloud_state/roles.csv`, 161 rows: 2 Comcast, 1 Jobgether — a query's location and an agency stamped as employers), and they are **still there, in master and on Pages**: the retraction path (`withdrawn` status, `cloud_state/roles_retractions.jsonl`, three purge sources, `roles_archive.csv`) is BUILT and reaches the public file only when the 2026-08-31 05:00 digest's publish step runs. Seniority is populated (0 empty of 172; it was empty on all 154 at 05:xx). Target 0 wrong rows, read from Pages. `curl -s https://analystjobsil.github.io/board/roles.csv \| grep -c 'Comcast\|Jobgether'` (3 on 08-30; 0 is the 08-31 morning check) — *(owns: the role as an ENTITY: is it the same one, is it still open, was it re-posted, when does it leave the board)* | **`pipeline/roles.py`** (the ledger: `cloud_state/roles.jsonl` + `roles_text.jsonl`), `pipeline/store.py` (`matched`/`sent`, `merge_key`, `seen_id`, `merge_duplicates`, `filter_new`, `upsert_matched`), the role-selection block in `pipeline/run.py`, repost detection |
