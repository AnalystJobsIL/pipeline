...
| **`roles`** | 6 | **11 published rows carried `class_decision=reject`, and 47 closed rows carry a verdict nothing re-judges** (re-derived 2026-09-18). The 2026-09-11 reading this replaces — 4 page-closed rows, 5 card-blob keys, 38 frozen verdicts — is **0, 0 and 47**: the first two classes closed (the `586` arm has closed 9 records by page, and `closed_by=page` names every one), the third GREW, which is what a growing store does when nothing drains it. The reject class is the one to read: a row published with a NO on it is neither IN under the live contract nor excluded with a reason (memory `dataset-acceptance-test`). Fixed today — `Ledger._withdraw_rejected` withdraws it the same run and `build_rows` refuses it as a tripwire — but **not yet proven by an unattended run**, so this reads 11 until the 2026-09-19 rows answer. The 47 are `classifier`'s to drain and **11 of them cannot be reached by any writer** (`626`). Target 0 and falling. `python -c "import json,csv,sys;sys.path.insert(0,'.');print(sum(1 for r in csv.DictReader(open('cloud_state/roles.csv',encoding='utf-8')) if r['class_decision']=='reject'),'published-reject;',sum(1 for l in open('cloud_state/roles.jsonl',encoding='utf-8') if l.strip() and json.loads(l).get('status')=='closed' and (json.loads(l).get('class') or {}).get('decision')=='accept'),'frozen')"` — *(owns: the role as an ENTITY: is it the same one, is it still open, was it re-posted, when does it leave the board)* | **`pipeline/roles.py`** (the ledger: `cloud_state/roles.jsonl` + `roles_text.jsonl`), `pipeline/store.py` (`matched`/`sent`, `merge_key`, `seen_id`, `merge_duplicates`, `filter_new`, `upsert_matched`, `rekey_matched`), the role-selection block in `pipeline/run.py`, repost detection |
