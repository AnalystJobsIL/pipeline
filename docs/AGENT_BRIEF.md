...
| **`company-intel`** | 4 | **10 active rows with no sector, by `identity_key`** (re-derived 2026-09-11 evening; the render-set gauge the mail prints reads the same 10). It read 21 on 08-30, and the number is a CADENCE before it is a capacity: every cron that adds a row moves it and no push can, which is why `tests/test_company_intel.py` measures and prints it rather than capping it. **All ten were one class until today** — a name whose board belongs to another company, held by the `employer_name` echo guard, struck, gated for 7 days and re-asked the identical question for ever; the mail now carries `N held (…)` and `N board-names-other` beside the level. Count by `identity_key`, never by exact name, which OVERSTATES it (84 exact / 68 by key on 08-30). Target 0. `python -c "import json,csv;from pipeline.firmographics import identity_key as k;f=json.load(open('cloud_state/firmographics.json',encoding='utf-8'));s={k(n) for n,v in f.items() if (v.get('sector') or '').strip()};print(sum(1 for r in csv.DictReader(open('companies.csv',encoding='utf-8')) if r['active'].strip().lower()=='true' and k(r['company_name']) not in s))"` — *(owns: sector / stage / employees / founded / Israel centre)* | `pipeline/firmographics.py`, `pipeline/company_info.py`, `research_firmographics.py`, `bd_employees.py`, `fill_employees_llm.py`, `company_type_analysis.py` |
