...
| `[wayback] submitted N, failed M, backlog K, boards B (of D due), verified V, throttled T, requests R, captured C, net a, server b, refused c[, stopped X][, host parked H]` | `archive_evidence.py`, the first step of `jd-archive.yml` (12:30), not the digest | what reached the Internet Archive today and how far behind the ledger runs. `captured` is what the archive NAMED (`submitted` counts timeouts too); `net`/`server` is the archive down, `refused` is the archive refusing our address, `stopped archive-down`/`throttled` says the day ended on a streak or a third 429 (2026-09-15). The same numbers are the `wayback` stamp in `Stage order:`, and a `wayback …` clause on `Stages:` means the stamp is two days old, a night named nothing (`zero-produce: 0 named of N planned (accepted M, net a, server b, refused c)`), the archive's daily limit bit, or the step crashed (`wayback crashed:<Exc>`) |
