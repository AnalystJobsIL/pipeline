...
| `[wayback] submitted N, failed M, backlog K, boards B (of D due), verified V, throttled T, requests R, captured C, net a, server b, refused c[, stopped X][, host parked H][, jobs J, polls P]` | `archive_evidence.py`, the first step of `jd-archive.yml` (12:30), not the digest | what reached the Internet Archive today and how far behind the ledger runs. `captured` is what the archive NAMED (`submitted` counts timeouts too); `net`/`server` is the archive down, `refused` is the archive refusing our address, `stopped archive-down`/`throttled` says the day ended on a streak or a third 429 (2026-09-15); `jobs`/`polls` (authenticated rung, 2026-09-16) are the SPN2 jobs opened and the status reads spent on them — never in `requests`. The same numbers are the `wayback` stamp in `Stage order:`, and a `wayback …` clause on `Stages:` means the stamp is two days old, a night named nothing (`zero-produce: 0 named of N planned (accepted M, net a, server b, refused c)`), the run had no keys or the archive refused them (`unauthenticated: <why>`, composed with the other clauses `; `-joined), the archive's daily limit bit, or the step crashed (`wayback crashed:<Exc>`) |
| `[wayback] authenticated: N slots, D of L captures used today; W workers, day cap C` and, after the day, `… captures used today (+K this run)` | `archive_evidence._probe` and `run()` | the account read from `/save/status/user` before the plan (its `available` slots bound the workers, what it has left bounds the day) and after it: `K` is the archive's own count of the jobs this run opened and should equal `jobs` — every job counts, an `error` end included (09-16: +13 for 13 jobs, 4 captured), so it is NOT `captured`. `[wayback] UNAUTHENTICATED: <why> -- anonymous rung` is the fallback, and `<why>` is also the alarm clause in the mail (2026-09-16) |
| `[wayback] pause k of 2 (throttled|archive-down): S s after R requests (net a, server b)` | `archive_evidence._Pool._pause` | the day paused: a 429 for its Retry-After, or 8 archive-side requests in a row for 90 s; the third of either ends the day. `[wayback] <host>: 5 consecutive refusals, parked for today` is now only ever the archive's word about that host's addresses (2026-09-15: it had fired on 0 refusals in twelve nights, every park was the archive down) |
