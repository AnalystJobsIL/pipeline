...
228. **Five real Israeli openings disappeared from the cache with the junk, and one wall is
...
229. **A converted or re-pointed row's `careers_url` no longer publishes its query string** —
    lane: `ats-fetch`, **closed 2026-08-26**. `cloud_state/stale.json` is a tracked file in the
    PUBLIC repo and `health.record` wrote `careers_url` as the row's `api_url` verbatim: the
    committed file carried **36 query strings, 9 of them Comeet `?token=` values** (AI21 Labs,
    Argus, Explorium, Foretellix, Frontegg, Hunters, Scopio Labs, Trullion, XM Cyber), while
    §5a's redaction sentence covered only the exception text. `health._public` now cuts the query
    the same way, and the committed file was rewritten in the same commit (0 remaining).
    A Comeet read token is public — the widget hands it to every visitor — so nothing private was
    disclosed; the file's own rule simply did not do what it said. **Note for `registry`:**
    ...
    all that rung does, that is neutral or better.
230. **The `misconfig-scrape-on-ats` queue cannot be drained by the tool that owns it: the comeet
    ...
    `www.comeet.com/jobs/<slug>/<uid>` — Comeet's own hosted board, not a company page with an
...
231. **`resolve_broken._HTML_ATS` has no comeet / bamboohr / workable rung** — lane: `registry`.
    ...
    the fetchers already exist, and `pipeline/platform_check.py` counts the same 7 missing
    `resolve_broken` cells.
232. **Five `alias-of` twins of `Amazon` and `Microsoft` are active scrape rows on ATS hosts** —
...
235. **The scraper's Comeet-widget title tail still reaches `store.merge_key`, so one posting can
    hold two role records** — lane: `scraper` (+ `roles`). 31 of the 47 cached titles on the 15
...
236. **The fetch loop is the largest block of the pipeline step again — because the classify
    ...
    (onsemi 10.8 s, Fortinet 10.1, Verint 4.5 for 49 jobs, Dell 39.2, JPMorganChase 14.9) —
    short-circuiting the pass once `TotalJobsCount` is reached saves ≈6 s of ~80 s over the five
...
237. **`tests/rehearse_infra.py`'s golden check has a dead allow-list, and §5's baseline cell is
    ...
    misleading message. And `ARCHITECTURE.md:1300` calls the baseline "monotonic — never
