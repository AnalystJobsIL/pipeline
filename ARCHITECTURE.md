...
| on push | tests | four jobs, no `continue-on-error` anywhere: `guard` (`pytest`, which runs `docs/check_docs.py`; `check_invariants.py`; `pipeline.platform_check`), `guard-kill` (a new test must fail without its fix), `rehearse` (six jobs, one per registry rehearsal: `worst` and `mixed` seeds 1–5, 14 nights each), `mutation-gate` (**twenty** shards, **by record**: `tools/mutate.py --all --shard I/N`, every N-th id -- the class-packed split of 08-28 could not divide `M1-gate-removal`, one class of 89 records, and shard 0 was killed at its budget on every push until 2026-08-30, BACKLOG 476; five shards walled **2,014-2,317 s** on 2026-08-31 and two were killed at 40 min, so the split went to eight on 09-01 at a measured **41.3 s per record**, and to **twenty on 2026-09-18** (`631`) when six of eight were killed at the budget with **zero surviving mutants** — the catalogue had gone 260 → 419 and the suite 1,766 → 2,083 tests, and since every record runs a SUBSET of the suite the per-record cost grew to **53.5 s** over a 172-210 s baseline, so 22 records a shard is the most that sits 20 % under the 1,800-s line. A shard past `MUTATE_WALL_WARN` (1,800 s) prints an `::warning::` on its own run page, which is the mechanism that names the next split: the ceiling returns at ~594 records, about a week at the 25 a day five lanes filed on 09-18). **Every long step runs under `timeout` with a budget below its job's `timeout-minutes`**, so an overrun is a failed step that names what it was running, never a cancelled job that names nothing (2026-08-30, below) |
...
| `cloud_state/pipeline_stages.json` | which nightly stage last finished and how much it did (`pipeline/stages.py`) — the digest alarms in the mail when a prerequisite stage did not run today. A stamp over a file that exists and does not parse is REFUSED with a `::warning::` (BACKLOG 451): a writer never rebases on `{}` | listing-hunt (`repair`, `queue`), scrape-refresh (`collect`, with its counts), auto-expand (`expand`), firmographics (`firmo`, `alarm=step-failed` when the step died from outside), jd-archive (`wayback`: `submitted`/`failed`/`backlog`/`boards`/`verified`/`throttled`, since 2026-09-15 `captured` (the archive NAMED a capture — `submitted` counts timeouts) and the failure split `net`/`server` (the archive down) vs `refused` (the archive refusing our address), `stop=` when the day ended early (`throttled` / `archive-down` / `daily-limit` / `unauthenticated`); `alarm=` on a night that named no capture, the archive's daily limit, or a crash — since 2026-09-04; since 2026-09-16 `auth` (`keyed` / `anon`), `jobs`, `polls`, and the alarm's clauses compose `; `-joined, so an unauthenticated zero night says `unauthenticated: …; zero-produce: …`), the digest (`enrich` via `jdfill.record_enrich`, `intel`, `publish`, and since 2026-08-30 `ci` and `cron` from its two alarm steps) — **merged per stage key on a conflict** (§4; until 2026-08-25 a conflict deleted other jobs' stamps) |
| `cloud_state/wayback_ledger.jsonl` | **one line per Save Page Now attempt** (2026-09-04): `at`, `url`, `kind` (`posting`/`board`), `tier`, `attempt`, `http`, `snap` (the 14-digit capture timestamp when the archive named one), `err` (`""` captured · `pending` — on the anonymous rung a 200 that named no capture, asked of the availability API from the next run; on the authenticated rung (2026-09-16) a job whose end this run did not read, asked of `/save/status/<job_id>` from the next run · `verified` that answer · `cached` · `throttled` · `server` · `net:<class>:<reason>` (the reason's class since 2026-09-15; 137 bare `net:URLError` lines said nothing) · `http` · `blocked` · `excluded` · `limit-url` · `daily-limit` · `unverified`), and since 2026-09-16, only when set, `job_id` (the SPN2 job) and `status_ext` (the archive's own word for an `error` end, e.g. `error:bad-request`, mapped onto `err` by `archive_evidence.STATUS_EXT`). ~170 bytes a line (~230 with a job), ~300 a day, and **never any page text** — the text stays in the archive, which is the point (the roles-text decision, `docs/decisions/2026-08-31-roles-text-artefact.md`). A failure is a cooldown (1 day for a throttle/5xx/connection error, 7 for another 4xx, 30 for an exclusion or past four REFUSALS — a 4xx, a block, an `unverified` capture; the archive's own failures are not the address's attempts since 2026-09-15, when 160 of 483 uncaptured urls had only those — or past twelve attempts of any kind), never a verdict. `archive_evidence.py` reads it back to know what is done, so it is the one `.jsonl` here that is NOT capped on a merge | `archive_evidence.py`, from jd-archive (12:30) |
...
accepted without naming the capture; the next run asks the archive — the job's own status
when the line carries a `job_id` (exact, the authenticated rung), else the availability API
— and writes `verified`. Postings are submitted oldest-first within tiers (the role store,
then the discovery net, then the scrape corpus) at `WAYBACK_DAY_CAP` a day (250 since
2026-09-16, 150 from 09-04, 100 before), a fifth of it reserved for addresses refused before
— and the discovery net alone adds ~124 a day (measured 2026-09-04), so `backlog` in the
`wayback` stamp is the queue; the archive allows an account 7 captures a minute, which is
the ceiling the cap sits under (the decision record has the arithmetic), and the ledger's
`throttled` and `daily-limit` lines say when the archive disagrees.

**Reading an authenticated night (2026-09-16).** The step log's first line is
`[wayback] authenticated: N slots, D of L captures used today; W workers, day cap C` — the
account read before the plan — and the same line after the day with `(+K this run)`, the
archive's own count of the jobs the run opened; `K` should equal the stamp's `jobs`, not
`captured` — every job counts, an `error` end included (09-16: +13 for 13 jobs, 4 captured).
`auth=anon` in the stamp with a `wayback unauthenticated: <why>` clause in the mail means
the keys did not reach the step or the archive refused them: the run fell back to the
anonymous rung and its numbers are that rung's. `stop=unauthenticated` is the archive
...
counts a timeout, and every one of the 271 `pending` lines to 09-15 was a 60-s timeout, of
which 206 never became a capture. `stop=archive-down` is 8 archive-side requests in a row,
three times, with two 90-s pauses between (`OUTAGE_AFTER`, `OUTAGE_WAIT_S`,
`PAUSE_EPISODES` in `archive_evidence.py`); `stop=throttled` is two 429 pauses and a third
429. Neither is a block on us — a block reads `refused`. On 09-14 the whole archive was down
(39 of 40 availability lookups failed too); on 09-15 only Save Page Now was (40 of 40
lookups answered, every `/save/` failed). The archive's failures come in runs — in the twelve
nights to 09-15 the five degraded ones ran 12-50 archive-side lines in a row, the seven
healthy ones at most 5 — which is why the day pauses on a streak and never parks a host for
one. The anonymous rung named ~9 captures a night for us (52 named + 53 verified in twelve
nights); the account's rung is bounded by the archive's 7 a minute instead (`620`).
...
one (`583@roles` carries the durable fix). Measured and REFUSED, with the numbers: the
...
suffix moves 11 of 94 titles, 10 refusal-to-refusal and 1 hearing lost, so that is `584@scraper`
...
| ...and the part no cap can reach | `classify N superseded verdicts CANNOT be re-judged (<why> n, …): the role has no description this run … (lane: jd-text)` — grouped by the `_jd_why` jd-text's `JDFiller.maybe_fill` leaves on the job (`not-a-job-url`, `wrong-address`, `bd-render-capped`, …, and `no-text-unattempted` — `seniority.NO_JD_WHY` — for a card jd-fill never tried, which printed a bare `?` until 2026-09-18), and each per-key step-log line ends `- jd: <why>` | same |
...
canonical key is taken`. It is measured EMPTY on the committed store and stays empty until some pair carries BOTH declarations the `declared` gate needs — a parked row's own dated `alias-of` verdict AND an `ALIASES` entry. **The Harel pair acquired both within hours of this landing**, which is the worked example: `ats-fetch` built `fetch_adamtotal` (the `career.adamtotal.co.il` board the identity gate could not admit, `621`; 83 of 83 IL postings), `registry` activated `Harel Insurance & Finance` on it and declared `הראל ביטוח ופיננסים` beside it. `_alias_fold_target` now answers `declared` for that name, and its one open role (`אנליסט ית תחקור ובקרה`) shares its title with no Harel record — so it takes the RENAME branch, not a twin fold. Before the rename existed it would have been left in place, stopped being fed, closed as if the posting had gone, and re-minted under the canonical key with a fresh `first_seen`. `merge_key` itself never
...
| `blurb-names-other A→B` | A's About text names employer B and not A — counted, never dropped (acquirers and customers are named legitimately; company-intel owns the blurb). **Who may ACCUSE and who may be NAMED are two questions, and one token dict answered both until 2026-09-19** (`632`): A excuses its own blurb by any spelling A writes — its registry key, any `firmographics.ALIASES` key that folds into it (`DoiT` on the row `doitintl`), or the brand on its card (`display_name`, already vetted there and already revoked by display-collision) — while B is named only when B's tokens mean B alone: a company whose one token is an ordinary word (Global-e, Port, Meta, Rise) or the second word of a two-word brand (`Air Products`→`products`, `Poc System`→`system`, `Google Israel`→`google` after `_ID_SUFFIX`) is named by nobody, and still accuses under its own name (`Oak - Identity→Sckipio`). Measured over the 188 cached blurbs that carry text: **6 hits → 2**, both real registry duplicates, and the board's own hit 1 → 0. `python tests/rehearse_render.py --blurbs` re-derives it; `--cards` reads the surface | `company-intel` |
