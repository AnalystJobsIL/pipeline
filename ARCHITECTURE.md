...
And **24 of the 76 workflow steps carry `continue-on-error: true`** (counted 2026-08-24 by
...
- "Did the run actually work?" → `gh run view <id> -R AnalystJobsIL/pipeline --log`.
  **24 of the 76 workflow steps are `continue-on-error`, so a green run can still hide a
